		...
		for(var j = 0; j <= World.RADIUS * 2; j++) {
			for(var i = 0; i <= World.RADIUS * 2; i++) {
				...
				if(World.curPos[0] == i && World.curPos[1] == j) {
					mapString += '<span class="landmark"><img src="./img/tileset/@.png" class="mapimg"/><div class="tooltip ' + ttClass + '">'+_('Wanderer')+'</div></span>';
				} else if(World.state.mask[i][j]) {
					var c = World.state.map[i][j];
					switch(c) {
						case World.TILE.VILLAGE:
							mapString += '<span class="landmark"><img src="./img/tileset/' + c + '.png" class="mapimg"/><div class="tooltip' + ttClass + '">'+_('The&nbsp;Village')+'</div></span>';
							break;
						default:
							if(typeof World.LANDMARKS[c] != 'undefined' && (c != World.TILE.OUTPOST || !World.outpostUsed(i, j))) {
								mapString += '<span class="landmark"><img src="./img/tileset/' + c + '.png" class="mapimg"/><div class="tooltip' + ttClass + '">' + World.LANDMARKS[c].label + '</div></span>';
							} else {
								if(c.length > 1) {
									c = c[0];
								}
								mapString += '<div style="position:relative;width:7px;height:7px;background-image:url(./img/tileset/123' + c  + '.png);background-size:cover;display: inherit;"></div>';
							}
							break;
					}
				} else {
					mapString += '<img src="./img/tileset/space.png" class="mapimg"/>';
				}
			}
			mapString += '<br/>';
		}
